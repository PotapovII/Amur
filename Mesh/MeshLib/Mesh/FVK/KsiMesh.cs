...
//        internal void TransportCVstructure(KsiMesh prevMesh)
//        {
//            if (prevMesh != null)
//            {
...
//            Mesh.Lx32 = new double[CountKnots][];
//            Mesh.Ly01 = new double[CountKnots][];
//            Mesh.Ly23 = new double[CountKnots][];
//            S = new double[CountKnots][];
//            Mesh.S0 = new double[CountKnots];
//            Mesh.Alpha = new double[CountKnots][];
//            Lk = new double[CountKnots][];

//            Mesh.P1 = new int[CountKnots][];
//            Mesh.Nx = new double[CountKnots][];
//            Mesh.Ny = new double[CountKnots][];
...
//                Mesh.Lx32[p0] = new double[jj + 1];
//                Mesh.Ly01[p0] = new double[jj + 1];
//                Mesh.Ly23[p0] = new double[jj + 1];
//                Mesh.Alpha[p0] = new double[jj + 1];
//                S[p0] = new double[jj + 1];
//                Lk[p0] = new double[jj + 1];
//                Mesh.P1[p0] = new int[jj + 1];
//                Mesh.Nx[p0] = new double[jj + 1];
//                Mesh.Ny[p0] = new double[jj + 1];
...
//                    int v2 = CVolumes[i][j + 1];
//                    //вторая точка общей грани
//                    Mesh.P1[p0][j] = SharedGrane(p0, v1, v2);
//                    //координаты рассматриваемого контура
//                    double x0 = CoordsX[p0]; double y0 = CoordsY[p0]; double x1 = CoordsX[Mesh.P1[p0][j]]; double y1 = CoordsY[Mesh.P1[p0][j]];
//                    double x2 = Xc[v1]; double y2 = Yc[v1]; double x3 = Xc[v2]; double y3 = Yc[v2];
//                    //длины плеч конечно-разностного аналога
//                    double lx10 = x1 - x0;
//                    double lx32 = x3 - x2;
...
//                    Mesh.Alpha[p0][j] = Math.Abs((x0 - xcr) * ly23 + (y0 - ycr) * lx32) / (Math.Abs((x0 - xcr) * ly23 + (y0 - ycr) * lx32) + Math.Abs((xcr - x1) * ly23 + (ycr - y1) * lx32));
//                    //площадь разностного аналога (не треугольника!)
//                    S[p0][j] = (lx10 * ly23 + lx32 * -ly01) / 2.0;
//                    //Вся площадь ячейки КО
//                    Mesh.S0[p0] += ((x3 - x0) * (y2 - y0) - (x2 - x0) * (y3 - y0)) / 2.0f;
//                    //if (Mesh.S0[i] < 0)
//                    //    Mesh.S0[i] = Mesh.S0[i];
//                    //длина текущего фрагмента внешнего контера КО
//                    Lk[p0][j] = Math.Sqrt(lx32 * lx32 + ly23 * ly23);
//                    //внешняя нормаль к грани КО (контуру КО)
//                    Mesh.Nx[p0][j] = ly23 / Math.Sqrt(lx32 * lx32 + ly23 * ly23);
//                    Mesh.Ny[p0][j] = lx32 / Math.Sqrt(lx32 * lx32 + ly23 * ly23);
//                    //
//                }
//            }
//            #endregion
