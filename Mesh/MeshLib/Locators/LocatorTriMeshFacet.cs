...
namespace MeshLib.Locators
{
    ...
    public class LocatorTriMeshFacet
    {
        ...
        {
            this.p0 = new HPoint(Points[0]);
            this.p1 = new HPoint(Points[1]);
            Dx = p1.X - p0.X; 
            Dy = p1.Y - p0.Y;
            line = new Vector2(p1 - p0);
            tau = Vector2.Normalize(line);
            normal = tau.GetOrtogonalLeft();
        }
    /// <summary>
    ...
    public List<TargetLinePoint> GetCrossLine(double[] H)
        ...
        /// <param name="x">аргумент функции - натуральный параметр створа</param>
        /// <param name="y">значение функции H в точках аргумента</param>
        public void GetCurve(double[] mVx, double[] mVy, ref double[] s, 
            ref double[] Vx, ref double[] Vy, ref double[] Vn, ref double[] Vt)
        {
            GetCurve(mVx, ref s, ref Vx);
            GetCurve(mVy, ref s, ref Vy);
            MEM.Alloc(s.Length, ref Vn);
            MEM.Alloc(s.Length, ref Vt);
            for (int i = 0; i < s.Length; i++)
            {
                Vn[i] = normal.X * Vx[i] + normal.Y * Vy[i];
                Vt[i] = tau.X * Vx[i] + tau.Y * Vy[i];
            }
        }
    }
}
