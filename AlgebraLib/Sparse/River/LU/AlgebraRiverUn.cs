...
namespace NPRiverLib.APRiver2XYD.River2DSW
{
    ...
    /// </summary>
    [Serializable]
    public class AlgebraRiverUn : IAlgebra
    {
        ...
        /// Название метода
        /// </summary>
        public string Name { get { return "River2D LU в формате CRS"; } }
        /// <summary>
        /// порядок СЛУ
        /// </summary>
        ...
        /// <summary>
        /// Количество неизвестных в узле Qx Qy H
        /// </summary>
        ...
        public AlgebraRiverUn()
        {
        }

        public AlgebraRiverUn(IMesh mesh)
        {
            Set(mesh);
        }
        public AlgebraRiverUn(AlgebraRiverUn ra)
        {
            Set(ra.mesh);
        }
        ...
        protected void Init()
        {
            ...
            //MEM.Alloc((uint)mesh.CountKnots, ref mpdArray, "mpdArray");
            ...
            for (i = 0; i < mesh.CountElements; i++)
            {
                ...
                if (d > 0)
                {
                    if (d > mpdArray[ements[i].Vertex3])
                        mpdArray[ements[i].Vertex3] = (uint)d;
                }
                else
                {
                    d *= -1;
                    if (d > mpdArray[ements[i].Vertex2])
                        mpdArray[ements[i].Vertex2] = (uint)d;
                }
                ...
                {
                    d *= -1;
                    if (d > mpdArray[ements[i].Vertex1])
                        mpdArray[ements[i].Vertex1] = (uint)d;
                }
        ...
        public void AddToMatrix(double[][] LMartix, uint[] AdressUn)
        {
            //int i, j, ii, jj;
            ...
            //}
            uint row, col;
            for (int i = 0; i < AdressUn.Length; i++)
            {
                row = AdressUn[i];
                for (int j = 0; j < AdressUn.Length; j++)
                {
                    col = AdressUn[j];
                    if (col >= row)
                        Kp[diagp[col] - col + row] += LMartix[i][j];
                    if (col <= row)
                        Lp[diagp[row] - row + col] += LMartix[i][j];
                }
            }
        }
        /// <summary>
        /// // Сборка ГПЧ
        /// </summary>
        public void AddToRight(double[] LRight, uint[] AdressUn)
        {
            for (int i = 0; i < AdressUn.Length; i++)
                Fp[AdressUn[i]] += LRight[i];
        }
        /// <summary>
        ...
        public void solve(ref double[] X)
        {
            ...
            for (uint j = 1; j < FN; j++)
            {
                ...
                // Проверка на выполнение прямого хода
                if (code < 2)
                {
                    iLp = diagp[j - 1] + 1;
                    id = (j - nf + 1);
        ...
        {
            return new AlgebraRiverUn(this);
        }
        /// <summary>
