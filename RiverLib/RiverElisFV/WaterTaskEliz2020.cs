...
namespace RiverLib
{
    ...
    /// </summary>
    [Serializable]
    public class WaterTaskEliz2020 : WElizParameter, IRiver
    {
        /// <summary>
        ...
        /// </summary>
        public WaterTaskEliz2020() : base(new WElizParameter())
        {
            SetModels();
        }
        /// <summary>
        ...
        /// </summary>
        public WaterTaskEliz2020(WElizParameter p) : base(p)
        {
            SetModels();
        }
        /// <summary>
        ...
        {
            name = "плоский турбулентнй поток (k-e) Елизарова";
            // функции стенки
            switch (wallFunctionType)
            {
        ...
        /// </summary>
        public string VersionData() => "WElizParameter 26 06 2017 "; 
        /// <summary>
        /// флаг наала задачи
        /// </summary>
        ...
        {
            base.SetParams((WElizParameter)p);
            SetModels();
        }
        /// <summary>
        ...
        public void Set(IMesh m, IAlgebra algebra = null)
        {
            ...
            if(algebra != null)
                ...
                this.algebra = new AlgebraGaussTape((uint)CountKnots, (uint)BWidth);
        ...
        public IRiver Clone()
        {
            return new WaterTaskEliz2020(this);
        }
        /// <summary>
        ...
        public void GetTau(ref double[] tauX, ref double[] tauY, ref double[] Press, ref double[][] CS, StressesFlag sf = StressesFlag.Nod)
        {
            tauX = null;
            tauY = null;
            Press = null;
        ...
        /// </summary>
        /// <param name="MiddleFunction"></param>
        ...
        /// </summary>
        public virtual void SolverStep()
        {
        ...
        protected void InitSurf_1(double Hn, double I = 0.2)
        {
            ...
            double Uav = Q / Hn;
            int N = 50;
            if (velocityInProfile == VelocityInProfile.PorabolicProfile)
            {
        ...
        protected void InitSurf_2(double Hn, double I = 0.2)
        {
            ...
            switch (turbulentModelType)
            {
                ...
                case TurbulentModelType.Model_k_eG: // Расчет k-e вместе с пристеночной функцией по Жлуткову 2015
                    ...
                    dPdx = -(2 + N) / Hn / Hn * C_m * K[mesh.CountLeft / 2] * K[mesh.CountLeft / 2] / (E[mesh.CountLeft / 2] + 1.0e-15) * rho_w * Uav;

                    break;
                case TurbulentModelType.Model_k_w:
                    //dPdx = -(2 + N) / Hn / Hn * mu * Uav;

                    dPdx = -(2 + N) / Hn / Hn * K[mesh.CountLeft / 2] / W[mesh.CountLeft / 2] * rho_w * Uav;
        ...
        protected int CalkVelosity(int i)
        {
            ...
            for (int j = 0; j < jj; j++)
            {
                ...
                double Ldkdy = ((LKc1 - LKc2) * _lx10 + (K[Lp1] - K[p0]) * _lx32) / Ls2;
                //
                double wk = tau * (LUcr * Ldkdx + LVcr * Ldkdy - (LNucr * (2 * Ldudx * Ldudx + (Ldvdx + Ldudy) * (Ldvdx + Ldudy) + 2 * Ldvdy * Ldvdy)) - LWcr * LKcr * beta_z);
                double wx = tau * (LUcr * Ldudx + LVcr * Ldudy + 1.0 / rho_w * Ldpdx + 2.0 / 3.0 * Ldkdx);
                double wy = tau * (LUcr * Ldvdx + LVcr * Ldvdy + 1.0 / rho_w * Ldpdy + 2.0 / 3.0 * Ldkdy);
                double ws = tau * (LScr * (Ldudx + Ldvdy) + (LUcr + Ww * SinJ) * Ldsdx + (LVcr - Ww * CosJ) * Ldsdy);
                //
                //
                //расчет потоков
        ...
        protected int CalkKW(int i)
        {
            ...
            for (int j = 0; j < jj; j++)
            {
                ...
                double wx = tau * (LUcr * Ldudx + LVcr * Ldudy + 1.0 / rho_w * Ldpdx + 2.0 / 3.0 * Ldkdx);
                double wy = tau * (LUcr * Ldvdx + LVcr * Ldvdy + 1.0 / rho_w * Ldpdy + 2.0 / 3.0 * Ldkdy);
                double wk = tau * (LUcr * Ldkdx + LVcr * Ldkdy - (LNucrT * (2 * Ldudx * Ldudx + (Ldvdx + Ldudy) * (Ldvdx + Ldudy) + 2 * Ldvdy * Ldvdy) - beta_z * LKcr * LWcr));
                double sigma_d = 0;
                double kwkw = Ldkdx * Ldwdx + Ldkdy * Ldwdy;
                if (kwkw > 0)
                    sigma_d = sigma_d_0;
                double ww = tau * (LUcr * Ldwdx + LVcr * Ldwdy - (alpha * LWcr / LKcr * LNucrT * (2 * Ldudx * Ldudx + (Ldvdx + Ldudy) * (Ldvdx + Ldudy) + 2 * Ldvdy * Ldvdy) - beta * LWcr * LWcr + sigma_d / LWcr * kwkw));
                //
                double LconvK = -LUcr * LKcr * Lnx - LVcr * LKcr * Lny;
