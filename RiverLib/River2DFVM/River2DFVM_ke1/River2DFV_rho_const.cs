...
namespace RiverLib.Patankar
{
    ...
    /// </summary>
    [Serializable]
    public class River2DFV_rho_const : ARiver2DFV
    {
        #region Свойства
        /// <summary>
        /// версия дата последних изменений интерфейса задачи
        /// </summary>
        public new string VersionData() => "StremRNS 2D 17.12.2021";
        ...
        /// </summary>
        /// <param name="ps"></param>
        public River2DFV_rho_const(): base() { }
        /// <summary>
        /// конструктор
        /// </summary>
        /// <param name="ps"></param>
        public River2DFV_rho_const(Stream2FVParams ps):base(ps){}
        /// <summary>
        ...
        {
            base.Set(ps);
            name = "плоский турбулентнй поток (k-e) rho_w = const";
        }
        /// <summary>
        ...
        {
            return new River2DFV_rho_const(new Stream2FVParams());
        }
        #endregion
        ...
        public override void OnInitialData()
        {
            ...
            for (int i = 0; i < Nx; i++)
            {
                for (int j = 0; j < Ny; j++)
                {
                    ...
                    // кинематическая тв
                    double nu0 = (double)(Params.Cmu * tke[i][j] * tke[i][j] / (dis[i][j] + 1e-30));
                    mut[i][j] = nu0 + Params.nu;
                }
            }
            // ---------------------------------------------------------
        ...
        protected override void OnLookingGama(int IndexTask)
        {
            ...
            double kappa_w = SPhysics.kappa_w; 
                ...
                case 0: // При расчете скорости u
                    {
                        ...
                        for (i = 0; i < imax + 1; i++)
                        {
                            ...
                            for (j = 0; j < jmax + 1; j++)
                            {
                                ...
                                amt = Params.Cmu * rho_w * tke[i][j] * tke[i][j] / (dis[i][j] + MEM.Error10);
                ...
                case 1:  // При расчете скорости v
                    {
                        ...
                        for (j = 1; j < jmax + 1; j++)
                        {
                            ...
                            xplus[j] = Params.cmu4 * Math.Sqrt(tke[1][j]) * Dx[1][j] / nu;
                            // поправка вязкости на стенке
                            if (xplus[j] > 11.5)
                                gam[0][j] = nu * xplus[j] / (b_kappa_w * Math.Log(Params.E_wall * xplus[j])) + mu;
                            gam[imax][j]  = mu;
                        }
                        // StatisticaYplus();
                        break;
                ...
                case 3: 
                    {
                        if (Params.TemperOrConcentration == true)
                        {
                            ...
                            double b_kappa_w = 1.0 / kappa_w;
                            factor = 1.0 / Params.PrandtlС;
                            for (j = 0; j < jmax + 1; j++)
                            {
                                gam[imax][j] = mu * factor ;
                                
                                if (xplus[j] > 11.5)
                                    gam[0][j] = nu / Params.PrandtlС * xplus[j] / (b_kappa_w * Math.Log(Params.E_wall * xplus[j]) + Params.pfn);
                                else
                                    gam[0][j] = mu * factor;
                        ...
                        else
                        {
                            ...
                            for (j = 0; j < jmax + 1; j++)
                            {
                                gam[imax][j] = mu * factor / Params.Prandtl;
                                gam[0][j] = mu * factor / Params.Prandtl;
                                if (xplus[j] > 11.5)
                                    gam[0][j] = nu / Params.Prandtl_T * xplus[j] / (b_kappa_w * Math.Log(Params.E_wall * xplus[j]) + Params.pfn);
                                for (i = 1; i < imax; i++)
                                {
                                    gam[i][j] = (mut[i][j] + mu) * factor;
        ...
        protected override void OnSource(int ist, int jst, int IndexTask)
        {
            ...
            double rho_w = SPhysics.rho_w;
            double Ws = SPhysics.PHYS.Ws;
            double Cmu = Params.Cmu;
            double c1 = Params.c1;
            double c2 = Params.c2;
            double cmu4 = Params.cmu4;
            switch (IndexTask)
            {
