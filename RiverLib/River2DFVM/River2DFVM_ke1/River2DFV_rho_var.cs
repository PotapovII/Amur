...
namespace RiverLib.Patankar
{
    ...
    /// </summary>
    [Serializable]
    public class River2DFV_rho_var : ARiver2DFV
    {
        #region Свойства
        /// <summary>
        /// версия дата последних изменений интерфейса задачи
        /// </summary>
        public new string VersionData() => "River2DFV_rho_var 31.01.2022"; 
        #endregion
        ...
        /// </summary>
        /// <param name="ps"></param>
        public River2DFV_rho_var(Stream2FVParams ps)
        {
            Set(ps);
        }
        /// <summary>
        ...
        /// </summary>
        public override void Set(Stream2FVParams ps)
        {
            base.Set(ps);
            name = "плоский турбулентнй поток (k-e) rho_w = var";
        }
        /// <summary>
        ...
        {
            return new River2DFV_rho_var(new Stream2FVParams());
        }
        #endregion
        ...
        public override void OnInitialData()
        {
            ...
            for (int i = 0; i < Nx; i++)
            {
                for (int j = 0; j < Ny; j++)
                {
                    ...
                    tke[i][j] = (double)(Params.Coeff_k * v[i][j] * v[i][j]);
                    dis[i][j] = (double)(Params.Coeff_e * tke[i][j] * tke[i][j]);
                    double mmu0 = (double)(Params.Cmu * tke[i][j] * tke[i][j] / (dis[i][j] + 1e-30));
        ...
        protected override void OnLookingGama(int IndexTask)
        {
            ...
            switch (IndexTask)
            {
                case 0: // При расчете скорости u
                    {
                        ...
                        for (i = 0; i < imax + 1; i++)
                        {
                            ...
                            for (j = 0; j < jmax + 1; j++)
                            {
                                ...
                                amt = Params.Cmu * rho[i][j] * tke[i][j] * tke[i][j] / (dis[i][j] + MEM.Error10);
                ...
                case 1:  // При расчете скорости v
                    {
                        ...
                        for (j = 1; j < jmax + 1; j++)
                        {
                            ...
                            xplus[j] = Params.cmu4 * Math.Sqrt(tke[1][j]) * Dx[1][j] / mu;

                            // поправка вязкости на стенке
                            if (xplus[j] > 11.5)
                                gam[0][j] = mu * xplus[j] / (b_kappa_w * Math.Log(Params.E_wall * xplus[j])) + mu;
                ...
                case 3: // При расчете температуры ищем температуропроводность
                    {
                        double b_kappa_w = 1.0 / kappa_w;
                        factor = 1.0 / Params.Prandtl_T;
                        for (j = 0; j < jmax + 1; j++)
                        {
                            gam[imax][j] = mu * factor;
                            gam[0][j] = mu / Params.Prandtl;
                            if (xplus[j] > 11.5)
                                gam[0][j] = mu / Params.Prandtl_T * xplus[j] / (b_kappa_w* Math.Log(Params.E_wall * xplus[j]) + Params.pfn);
                            for (i = 1; i < imax; i++)
                            {
                ...
                case 5: // При расчете диссипации турбулентной кинитической энергии
                    {
                        ...
                        for (j = jst; j < jmax; j++)
                        {
                            ...
                            double diss = Params.Cmu * tke[ist][j] * Math.Sqrt(Math.Abs(tke[ist][j])) / (0.4 * Params.cmu4 * Dx[ist][j]);
                            // расчет источников для b и Ap
                            sc[ist][j] = (double)(WW * diss);
                            sp[ist][j] = (double)-WW;
                            for (i = ist + 1; i < imax; i++)
                            {
                                sc[i][j] = Params.c1 * gen[i][j] * Params.Cmu * rho[i][j] * tke[i][j];
                                sp[i][j] = -Params.c2 * rho[i][j] * dis[i][j] / (tke[i][j] + MEM.Error10);
                            }
                        }
                        break;
                    }
        ...
        ///// </summary>
        //public override void GetForce(ref double[] tau, ref double[] Pb, ref double[] Yplus)
        //{
        //    // Определение индексов границ для расчета донных деформаций
        //    qmesh.GetZetaArea(ref count, ref In, ref Out, Len1, Len2);
        //    MEM.Alloc<double>(count - 1, ref tau);
        //    MEM.Alloc<double>(count - 1, ref Yplus);
        //    // Настройка 
        //    WallFunction.E_wall = E_wall;
        //    WallFunction.nu = nu;
        //    WallFunction.rho = rho_w;
        //    // расчет напряжений
        //    int k = 0;
        //    double Tau = 0, yplus = 0;
        //    for (int j = In; j < count + In - 1; j++)
        //    {
        ...
        //        WallFunction.Tau(V, Y, ref Tau, ref yplus);
        //        tau[k] = Tau;
        //        Yplus[k] = yplus;
        //        k++;
        //    }
