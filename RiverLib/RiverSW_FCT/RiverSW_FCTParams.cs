...
namespace RiverLib
{
    ...
    /// <summary>
    ///  ОО: Параметры для класса RiverSW_FCT
    /// </summary>
    [Serializable]
    public class RiverSW_FCTParams
    {
        // ========================== Алгоритм  ========================== 
        /// <summary>
        ...
        public RiverSW_FCTParams(RiverSW_FCTParams p)
        {
            if (p != null)
                SetParams(p);
            else
                SetParams(new RiverSW_FCTParams());
        }
        public void SetParams(RiverSW_FCTParams p)
        {
            time = p.time;
            dtime = p.dtime;
            dtmax = p.dtmax;
