...
namespace NPRiverLib.APRiver_2XYD.River2DSW
{
    ...
    /// </summary>
    [Serializable]
    public class TriRiverSW_2XYD : APRiverFEM_2XYD
    {
        /// <summary>
        /// ограничители для ГУ
        /// </summary>
        ...
        public TriRiverSW_2XYD() : base(new ParamsRiver_2XYD())
        {
            name = "Плановая задача мелкой воды TriRiverSW_2XYD"; 
                    ...
                    RiverNode[] elemNodes, AbFunForm BFunN, double dx, double dy)
        {
            ...
            if ((bElement.boundCondType == 5) && ((Ha - tice) > Params.H_minGroundWater))
            {
                ex = bElement.Qn;
        ...
        protected double FunctionLimiters(RiverNode[] elemNodes, uint nod, uint i)
        {
            ...
            mult = 1.0;
            if (elemNodes[nod].fxc == 1)
            {
                if (nod == 0 && (elemNodes[1].fxc == 1))
                {
                    l1 = 0;
                    l2 = 1;
                ...
                if (nod == 0 && (elemNodes[nbn - 1].fxc) == 1)
                {
                    l1 = 0;
                    l2 = nbn - 1;
                    funforms.CalkVertex(l1, ref r1, ref s1);
                ...
                if (nod == (nbn - 1) && elemNodes[nbn - 2].fxc == 1)
                {
                    l1 = nbn - 1;
                    l2 = nbn - 2;
                    funforms.CalkVertex(l1, ref r1, ref s1);
                ...
                if (nod == (nbn - 1) && elemNodes[0].fxc == 1)
                {
                    l1 = nbn - 1;
                    l2 = 0;
                    funforms.CalkVertex(l1, ref r1, ref s1);
        ...
        public void BoundaryConditionUpdate() // update_BCs
        {
            ...
            for (int i = 0; i < meshRiver.CountSegment; i++)
            {
                ...
                meshRiver.boundSegment[i].Dcoef = 0;
                ...
                for (uint be = (uint)sID; ; be++)
                {
                    ...
                    double Length = meshRiver.GetBoundElemLength(be);
                    ...
                    double wse = meshRiver.FreeFlowSurfaceElement(be);
                ...
                for (int be = sID; ; be++)
                {
                    ...
                    if ((meshRiver.nodes[Vertex1].h - meshRiver.nodes[Vertex1].Hice * Params.iceCoeff) < Params.H_minGroundWater)
                        dryFlag = 1;
                    if ((meshRiver.nodes[Vertex2].h - meshRiver.nodes[Vertex2].Hice * Params.iceCoeff) < Params.H_minGroundWater)
                        dryFlag = 1;
                    // Вычисляем среднюю по граничному элементу глубину
                    meshRiver.BoundElems[be].H =  // p[3]
                         (meshRiver.nodes[Vertex1].h + meshRiver.nodes[Vertex2].h) * 0.5;
                    // Вычисляем среднюю по граничному элементу отметку дна
                    meshRiver.BoundElems[be].Zeta =  // p[4]
                        (meshRiver.nodes[Vertex1].zeta + meshRiver.nodes[Vertex2].zeta) * 0.5; // дно
                    double ice1 = meshRiver.nodes[meshRiver.BoundElems[be].Vertex1].Hice;
                    double ice2 = meshRiver.nodes[meshRiver.BoundElems[be].Vertex2].Hice;
                    ...
                    meshRiver.BoundElems[be].Eta = meshRiver.boundSegment[i].Hn;
                ...
                if ((meshRiver.boundSegment[i].boundCondType == 1) || (meshRiver.boundSegment[i].boundCondType == 2))
                {
                    for (int be = sID; ; be++)
                    {
                        if (be != sID)
                        {
                            if (meshRiver.BoundElems[be - 1].H < MEM.Error3)
                        ...
                        meshRiver.boundSegment[i].Dcoef += fac * Math.Pow(meshRiver.BoundElems[be].H, (5.0 / 3.0)) * meshRiver.BoundElems[be].Length;
                        if (meshRiver.BoundElems[be].H > 0)
                            meshRiver.BoundElems[be].H = meshRiver.BoundElems[be].H;
                        if (be >= eID)
                            break;
                    }
                }
                    ...
                    for (int be = sID; ; be++)
                    {
                        if (meshRiver.BoundElems[be].boundCondType == 1 || (meshRiver.BoundElems[be].boundCondType == 2))
                        {
                            ...
                            if ((Hice1 > 0) && (Hice2 > 0))
                                ...
                                fac = 1.0;
                            if (meshRiver.boundSegment[i].Dcoef > 0.0)
                            {
                                // распределение расхода в узлы пропорционально глубине потока
                                double normDcoef_i = fac * Math.Pow(meshRiver.BoundElems[be].H, (5.0 / 3.0)) / meshRiver.boundSegment[i].Dcoef;
                                meshRiver.BoundElems[be].Qn = normDcoef_i * meshRiver.boundSegment[i].Qn;
                                meshRiver.BoundElems[be].Qn = fac * Math.Pow(meshRiver.BoundElems[be].H, (5.0 / 3.0))
                                    * meshRiver.boundSegment[i].Qn / meshRiver.boundSegment[i].Dcoef;
                            }
                            else
                            {
        ...
        int CheckMixedRB(RiverNode[] elemNodes)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                mult = ((elemNodes[i].h0 - (elemNodes[i].Hice * Params.iceCoeff)) - Params.H_minGroundWater) *
                       ((elemNodes[j].h0 - (elemNodes[j].Hice * Params.iceCoeff)) - Params.H_minGroundWater);
                if (mult < 0.0)
                    return 1;
            }
            return 0;
        ...
        /// <param name="g"></param>
        /// <returns></returns>
        int GetBoundaryGPS(RiverNode[] elemNodes, ref NumInegrationPoints bpIntegration)
        {
            int ngauss = 0;
            double[] r = new double[2];
            double gwH = Params.H_minGroundWater;
            double a = (elemNodes[0].h - (elemNodes[0].Hice * Params.iceCoeff)) - gwH;
            double c = (elemNodes[1].h - (elemNodes[1].Hice * Params.iceCoeff)) - gwH;
        ...
        int GetMixedGPS(TriElementRiver currentFElement, ref NumInegrationPoints pIntegration)
        {
            ...
            double[] xTriangle = new double[CountElementKnots];
            double[] yTriangle = new double[CountElementKnots];
            double level = Params.H_minGroundWater;
            ...
            double level, ref int nRight, ref int nLeft, ref RiverNode[] right, ref RiverNode[] left)
        {
            ...
            for (i = 0; i < CountElementKnots; i++)
            {
                ...
                mult = ((nodes[i].h0 - (nodes[i].h_ise * Params.iceCoeff)) - level) *
                       ((nodes[nlast].h0 - (nodes[nlast].h_ise * Params.iceCoeff)) - level);
                ...
                if (mult == 0.0)
                {
                    if ((nodes[i].h0 - (nodes[i].h_ise * Params.iceCoeff)) == level)
                    {
                ...
                if (mult < 0.0)
                {
                    double factor1 = Math.Abs((nodes[i].h0 - (nodes[i].h_ise * Params.iceCoeff)) - level);
                    double factor2 = Math.Abs((nodes[nlast].h0 - (nodes[nlast].h_ise * Params.iceCoeff) - level));
        ...
        /// </summary>
        public void ShiftingTemporaryLayers()
        {
            for (int i = 0; i < meshRiver.CountKnots; i++)
            {
        ...
        public static void Test()
        {
            ...
            IRiver river2D = new TriRiverSW_2XYD();
        ...
        public double GetWsElev(ref BoundElementRiver inSegP, ref BoundElementRiver outSegP)
        {
            ...
            for(int i = 0; i < meshRiver.CountBoundElements; i++)
            {
                BoundElementRiver elem = meshRiver.BoundElems[i];
