...
namespace MeshExplorer
{
    public partial class FormMain : Form
    {
        ...
        private void HandleNewInput()
        {
            ...
            this.Text = "Triangle.NET - MeshNet Explorer - " + settings.CurrentFile;
        }
        ...
        private void Triangulate()
        {
            ...
            try
            {
                ...
                {
                    mesh = (MeshNet)input.Triangulate(options, quality, new SweepLine());
                }
                else
                {
                    mesh = (MeshNet)input.Triangulate(options, quality);
                }
        ...
        private void menuToolsCheck_Click(object sender, EventArgs e)
        {
            if (mesh != null)
            {
                ...
                if (isConsistent)
                {
                    Log.Instance.Info("MeshNet topology appears to be consistent.");
                }

                if (isDelaunay)
                {
                    Log.Instance.Info("MeshNet is (conforming) Delaunay.");
                }
