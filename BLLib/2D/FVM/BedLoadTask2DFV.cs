...
namespace BLLib
{
    ...
    /// </summary>
    [Serializable]
    public class BedLoadTask2DFV : BedLoadParams, IBedLoadTask
    {
        /// <summary>
        ...
        /// </summary>
        public virtual TypeTask typeTask { get => TypeTask.streamXY2D; }
        /// <summary>
        /// версия дата последних изменений интерфейса задачи
        /// </summary>
        public virtual string VersionData() => "BedLoadTask2DFV 18.06.2022"; 
        /// <summary>
        /// Сетка решателя
        /// </summary>
        ...
        protected IUnknown[] unknowns = { new Unknown("Отметки дна", null, TypeFunForm.Form_2D_Triangle_L1) };
        /// <summary>
        /// расчетная сетка
        /// </summary>
        public RectFVMesh mesh;
        /// <summary>
        ...
        public BedLoadTask2DFV(BedLoadParams p) : base(p)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IBedLoadTask Clone()
        {
            return new BedLoadTask2DFV(new BedLoadParams());
        }
        ...
        public void CalkZetaFDM(ref double[][] zeta, double[][] TauX, double[][] TauY, double[][] Press = null, double[][] CS = null) 
        {
            ...
            for (j = 1; j < jmax; j++)
            {
                for (i = 1; i < imax; i++)
                {
                    dZetadX = (zeta0[i + 1][j] - zeta0[i - 1][j]) / (mesh.Dx[i][j] + mesh.Dx[i - 1][j]) * mesh.hy[i][j];
                    dZetadY = (zeta0[i][j + 1] - zeta0[i][j - 1]) / (mesh.Dy[i][j] + mesh.Dy[i][j - 1]) * mesh.hx[i][j];
            ...
            for (i = ist; i < imax; i++)
            {
                ...
                for (j = jst; j < jmax; j++)
                {
                    double hy_hx = mesh.hy[i][j] / mesh.hx[i][j];
                    double hx_hy = mesh.hx[i][j] / mesh.hy[i][j];
                    ...
                    if (DryWet[i][j] == true)
                    {
                        ...
                        App0[i][j] = rho_s * (1 - epsilon) * mesh.hx[i][j] * mesh.hy[i][j] / dtime;
                        App[i][j] = App0[i][j];
                        sc[i][j] = App0[i][j];
                        sp[i][j] = 0;
                    }
                    else
                    {
                        ...
                        Aep[i][j] = 0.5 * (Sxx[i][j] + Sxx[i + 1][j]) / mesh.Dx[i][j] * hy_hx;
                        // Awp := 1/2*(Sxx_pp+Sxx_wp)/Dx_wp*Hy_pp/Hx_pp
                        Awp[i][j] = 0.5 * (Sxx[i][j] + Sxx[i - 1][j]) / mesh.Dx[i - 1][j] * hy_hx;
                        // Apn := 1/2*(Syy_pp+Syy_pn)/Dy_pp*Hx_pp/Hy_pp
                        Aps[i][j] = 0.5 * (Syy[i][j] + Syy[i][j + 1]) / mesh.Dy[i][j] * hx_hy;
                        // Aps := 1/2*(Syy_pp+Syy_ps)/Dy_ps*Hx_pp/Hy_pp
                        Apn[i][j] = 0.5 * (Syy[i][j] + Syy[i][j - 1]) / mesh.Dy[i][j - 1] * hx_hy;
                        // центр
                        App[i][j] = Aep[i][j] + Awp[i][j] + Apn[i][j] + Aps[i][j] + App0[i][j];
                        // Aen := Syx_pn/(Dx_pn+Dx_wn)*Hx_pp/Hy_pp+Sxy_ep/(Dy_ep+Dy_es)*Hy_pp/Hx_pp
                        Aen[i][j] = Syx[i][j + 1] / (mesh.Dx[i][j + 1] + mesh.Dx[i - 1][j + 1]) * hx_hy +
                                    Sxy[i + 1][j] / (mesh.Dy[i + 1][j] + mesh.Dy[i + 1][j - 1]) * hy_hx;
                        // Awn := -Syx_pn/(Dx_pn+Dx_wn)*Hx_pp/Hy_pp-Sxy_wp/(Dy_wp+Dy_ws)*Hy_pp/Hx_pp
                        Awn[i][j] = -Syx[i][j + 1] / (mesh.Dx[i][j + 1] + mesh.Dx[i - 1][j + 1]) * hx_hy +
                                   -Sxy[i - 1][j] / (mesh.Dy[i - 1][j] + mesh.Dy[i - 1][j - 1]) * hy_hx;
                        // Aes := -Syx_ps/(Dx_ps+Dx_ws)*Hx_pp/Hy_pp-Sxy_ep/(Dy_ep+Dy_es)*Hy_pp/Hx_pp
                        Aes[i][j] = -Syx[i][j - 1] / (mesh.Dx[i][j - 1] + mesh.Dx[i - 1][j - 1]) * hx_hy +
                                   -Sxy[i + 1][j] / (mesh.Dy[i + 1][j] + mesh.Dy[i + 1][j - 1]) * hy_hx;
                        ...
                        sc[i][j] = 0.5 * (A[i + 1][j] * CosGamma[i + 1][j] - A[i - 1][j] * CosGamma[i - 1][j]) * mesh.hy[i][j] +
                                   0.5 * (A[i][j + 1] * CosGamma[i][j + 1] - A[i][j - 1] * CosGamma[i][j - 1]) * mesh.hx[i][j] +
                                   App0[i][j];
                        sp[i][j] = 0;
                    }
                }
