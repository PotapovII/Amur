...
namespace BLLib
{
    ...
    /// </summary>
    [Serializable]
    public class Adapter1Dfor2D : ABedLoadFEMTask2D
    {
        public override IBedLoadTask Clone()
        {
            return new Adapter1Dfor2D(new BedLoadParams());
        }
        ...
        /// Конструктор 
        /// </summary>
        public Adapter1Dfor2D(BedLoadParams p) : base(p)
        {
            cu = 3;
        ...
        public override void SetTask(IMesh mesh, double[] Zeta0, IBoundaryConditions BConditions)
        {
            ...
            // инициализация 1D задачи
            //TypeBoundCond[] BCType = mesh.GetBoundElementsType();
            //double[] BCValue = BConditions.GetValueDir();
            //if (BConditions?.GetValueNeu().Length == 4)
            //{
            //    //bltask.BCondIn = new BoundCondition1D(BCType[3], BCValue[3]);
            //    //bltask.BCondOut = new BoundCondition1D(BCType[1], BCValue[1]);
            //    bltask.BCondIn = new BoundCondition1D(BCType[3], BCValue[3]);
            //    bltask.BCondOut = new BoundCondition1D(BCType[1], BCValue[1]);
            //}
            //else
            {
                bltask.BCondIn = new BoundCondition1D(TypeBoundCond.Neumann, 0);
