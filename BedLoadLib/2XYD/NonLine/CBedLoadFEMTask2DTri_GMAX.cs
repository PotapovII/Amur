...
namespace BedLoadLib
{
    ...
    /// </summary>
    [Serializable]
    public class CBedLoadFEMTask2DTri_GMAX : CBedLoadFEMTaskTri_2D
    {
        /// <summary>
        ...
        {
            return new CBedLoadFEMTask2DTri_GMAX(new BedLoadParams2D());
        }
        /// <summary>
        /// Конструктор 
        /// </summary>
        public CBedLoadFEMTask2DTri_GMAX(BedLoadParams2D p) : base(p)
        {
            ...
            b = new double[cu];
            ss = (1 + s) / s;
            name = "плановая деформация одно-фракционного дна с большими уклонами.";
        }
        /// <summary>
        ...
        public override void CalkCoeff(uint elem, double mtauS, double dZx, double dZy)
        {
            ...
            else
            {
                ...
                //double scaleX = core.CoreX(dZx, dZy);
                double scaleX = 1.0 / Math.Max(0.1, 1 - dZx * dZx) - 37.0 / 105 * (9.0 / 74 + dZx) * dZy * dZy / (Math.Max(0.1, 1 - dZx));
                //double scaleY1 = -5.0/4*core.CoreY(dZx, dZy);
                double scaleY =  1 - 8.0 / 7 * dZx + 25.0 / 21 * dZx * dZx + 17.0 / 42 * dZy * dZy;
                if (Params.blm != TypeBLModel.BLModel_1991)
                {
                    ...
                    {
                        dPX += a[ai] * press[ai] / (rho_b * GRAV);
                        dPY += b[ai] * press[ai] / (rho_b * GRAV);
                    }
                    //dPX /= (2 * S);
                    //dPY /= (2 * S);
