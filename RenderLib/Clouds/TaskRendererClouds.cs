...
namespace RenderLib
{
    ...
    /// </summary>
    public class TaskRendererClouds : TaskRenderer
    {
        double[] Values = null;
        double MaxV = 0;
        double MinV = 0;
        ...
        /// </summary>
        public EditRenderOptions editRenderOptions = new EditRenderOptions();
        /// <summary>
        ...
        /// </summary>
        public ColorSchemeClouds colorScheme = new ColorSchemeClouds();
        /// <summary>
        ...
        /// </summary>
        public TaskRendererClouds(IClouds clouds)
        {
        ...
        private void RenderPoints(Graphics g)
        {
            ...
            for (i = 0; i < X.Length; i++)
            {
                ...
                g.FillEllipse(colorScheme.BrushNodes, pt.X - 1.5f, pt.Y - 1.5f, 3, 3);

                if (renderOptions.showKnotNamber == true)
                {
                    PointF pp = new PointF(pt.X + 10f, pt.Y);
                    g.DrawString(i.ToString(), colorScheme.FontNodesCloud, colorScheme.BrushTextNodes, pp);
                }
            }
        }
        /// <summary>
        ...
        private void RenderValuePoints(Graphics g)
        {
            ...
            if (pole.Dimention == 1)
            {
                ...
                for (i = 0; i < X.Length; i++)
                {
                    pt = new PointF((float)X[i], (float)Y[i]);
                    zoom.WorldToScreen(ref pt);
                    pt.X += 10f; pt.Y = pt.Y - colorScheme.FontNodesCloud.Size - 3;
                    res = ": " + String.Format(colorScheme.FormatTextValues, ValuesX[i]);
                    g.DrawString(res, colorScheme.FontNodesCloud, colorScheme.BrushTextValues, pt);
                }
            }
            else
            {
                ...
                for (i = 0; i < X.Length; i++)
                {
                    ...
                    res = "> " + String.Format(colorScheme.FormatTextValues, ValuesX[i]);
                    g.DrawString(res, colorScheme.FontNodesCloud, colorScheme.BrushTextValues, pt);
                    pt.Y = pt.Y - colorScheme.FontNodesCloud.Size - 2;
                    res = "^ " + String.Format(colorScheme.FormatTextValues, ValuesY[i]);
                    g.DrawString(res, colorScheme.FontNodesCloud, colorScheme.BrushTextValues, pt);
                }
            }
        }
        ...
        private void RenderBoundaryKnots(Graphics g)
        {
            ...
            if (renderOptions.showBoudaryKnots == true)
            {
                ...
                for (int i = 0; i < X.Length; i++)
                {
                    ...
                    Color col;
                    if (MEM.Equals(MinV, MaxV) == true)
                        col = Color.Black;
                    else
                        col = colorScheme.RGBBrush(Values[i], MinV, MaxV);
            ...
            else
            {
                for (int i = 0; i < X.Length; i++)
                {
                    p0 = new PointF((float)X[i], (float)Y[i]);
                    zoom.WorldToScreen(ref p0);
                    g.FillEllipse(colorScheme.BrushNodes, p0.X - 1.5f, p0.Y - 1.5f, 3, 3);
                }
            }
        }
        ...
        private void RenderVectorField(Graphics g)
        {
            ...
            for (int i = 0; i < Values.Length; i++)
            {
                pt = new PointF((float)X[i], (float)Y[i]);
                zoom.WorldToScreen(ref pt);
                g.FillEllipse(colorScheme.BrushNodes, pt.X - 2.5f, pt.Y - 2.5f, 5, 5);
